...
def ensure_yt_dlp():
    """Ensure yt_dlp is installed and up to date."""
    try:
        import yt_dlp
    except ImportError:
...
def progress_hook(d, log_widget, stop_flag):
    """Handle progress updates and detect cancellations."""
    if stop_flag["cancel"]:
    ...
    # Record current filename for cleanup
    if filename:
        stop_flag["last_file"] = str(filename)

    if status == "downloading" and not filename.suffix in [".webm", ".tmp"]:
        ...
        if filename.suffix not in [".webm", ".tmp"]:
            log_widget.insert(tk.END, f"Merging files for {filename.stem}...\n")
            log_widget.see(tk.END)
    elif status == "postprocessing":
        if d.get("postprocessor") == "FFmpegMerger":
            log_widget.insert(tk.END, "Merging complete. Finalizing file...\n")
            log_widget.see(tk.END)


def cleanup_temp_files(output_dir, log_widget):
    """Remove leftover temporary download files."""
    for ext in [".part", ".webm", ".tmp"]:
        for f in Path(output_dir).glob(f"*{ext}"):
            try:
                f.unlink()
                log_widget.insert(tk.END, f"Removed leftover temp file: {f.name}\n")
            except Exception as e:
                log_widget.insert(tk.END, f"Could not remove {f}: {e}\n")
    log_widget.see(tk.END)
...
def download_media(url, format_choice, log_widget, music_dir, video_dir, stop_flag):
    ...
    if is_audio:
        ydl_opts.update({
            ...
            "postprocessors": [
                {"key": "FFmpegExtractAudio", "preferredcodec": format_choice.lower(), "preferredquality": "192"},
                {"key": "FFmpegMetadata"},
            ],
        })
    else:
    ...
    try:
        ...
        # Delete last partial file
        last_file = stop_flag.get("last_file")
        if last_file and Path(last_file).exists():
            try:
                os.remove(last_file)
                log_widget.insert(tk.END, f"Deleted incomplete file: {last_file}\n")
            except Exception as del_err:
                log_widget.insert(tk.END, f"Could not delete file: {del_err}\n")

        # Clean up any temp files that might remain
        cleanup_temp_files(output_dir, log_widget)
...
class ModernDownloaderApp(tk.Tk):
    def __init__(self):
        ...
        self.stop_flag = {"cancel": False, "last_file": None}
